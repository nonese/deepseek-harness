...
在已认证的多用户组合中，主体作用域视图会为普通用户收紧这一基础约定：`agentPreset.list` 移除随附的 `cordis` preset，并报告 `authorable: false` 与 `hasDocument: false`；`session.create({ agentPreset: 'cordis' })` 和 `agentPreset.select({ agentPreset: 'cordis' })` 都会在调用基础 API 之前失败。管理员保留完整名单与行为。工具执行和动态 runner 还各自具有独立的 Host 端管理员检查，因此 preset 过滤本身并非权限边界。
...
已认证部署可以用服务端统一凭据为模型授权，而不是使用提供方 profile 中的普通凭据。只有当调用方符合条件且统一凭据已配置时，主体级 `llm.providers` 响应才会在 `managedModels` 中列出这些模型 id；该字段绝不包含凭据内容。

## 载体层（`/client` + 根路径）

`AbstractApiClient` 持有全部协议不变量：签发 rpcId、包装／解包信封、Zod 解析、SSE 帧解码、一元请求超时，以及按微任务批处理的信封观测（`subscribeEnvelopes`）；平台子类只提供 `doFetch` 传输环节。UUID v4 签发使用 `crypto.getRandomValues()`，而非仅安全上下文可用的 `crypto.randomUUID()`，因此显式开放的普通 HTTP 局域网源在加入 TLS 前也能发起调用。`InProcessApiClient` 以 `toFetchHandler(api)` 为基础，仍是同构接点：它运行完整的协议序列化与校验路径而不经过网络，供需要该路径的调用方和载体测试使用。产品的 `dsh --profile headless` 是直连 core 的入口，不挂载本包。
