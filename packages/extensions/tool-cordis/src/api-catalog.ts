...
export const SERVICE_API: readonly ServiceApiEntry[] = [
  ...
  },
  {
    ...
    methods: [
      {
        signature: 'currentPrincipal(): AuthPrincipal | undefined',
        description: 'Read the principal established by the authenticated carrier for this async request.',
        parameters: [],
        returns: 'the current principal, or undefined outside an authenticated request.',
      },
      {
        signature: 'withPrincipal<T>(principal: AuthPrincipal, operation: () => T): T',
        description: 'Run one carrier dispatch with an immutable authenticated principal.',
        parameters: [{ name: 'principal', description: 'identity resolved from the deployment session cookie.' }, { name: 'operation', description: 'request work that may cross asynchronous continuations.' }],
  ...
  },
  {
    ...
    methods: [
      ...
      {
        signature: 'registerMiddleware(middleware: TypertGatewayMiddleware): () => Promise<void>',
        description: 'Register one effect-scoped Remote middleware in activation order.',
        parameters: [{ name: 'middleware', description: 'unary and stream dispatch wrapper to append.' }],
...
export const TYPE_API: readonly TypeApiEntry[] = [
  ...
  {
    name: 'TypertGatewayMiddleware',
    declaration: 'export interface TypertGatewayMiddleware {\n    invoke(request: InvokeRemoteRequest, next: () => Promise<unknown>): Promise<unknown>;\n    stream(request: InvokeRemoteRequest, next: () => Promise<AsyncIterable<unknown>>): Promise<AsyncIterable<unknown>>;\n}',
  },
  {
