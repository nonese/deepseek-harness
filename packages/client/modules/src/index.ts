...
export class ClientModuleRegistry extends Service {
  ...
  private locatePkgJson(loaderName: string, baseUrl: string): { path: string; packageName: string } | undefined {
    ...
    const resolveFromOwningTree = (): { path: string; packageName: string } | undefined => {
    ...
    try {
      ...
      // Source launch hooks can import a package while Node's internal
      // synchronous resolver cannot reproduce the hook result. The owning
      // profile tree still resolves the exact package manifest in that mode.
      return resolveFromOwningTree()
    }
    return this.nearestPackage(moduleUrl, expectedPackageName) ?? resolveFromOwningTree()
  }
