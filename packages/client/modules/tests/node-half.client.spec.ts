...
describe('client bundle activation', () => {
  ...
  it('falls back to the owning tree when the synchronous Loader resolver cannot see a source-launch package', () => {
    const packageName = '@fixture/source-launch-fallback'
    const clientPath = writePackage(packageName)
    mkdirSync(dirname(clientPath), { recursive: true })
    writeFileSync(clientPath, 'module.exports = {}\n')
    const internal = {
      version: 'v2' as const,
      resolveSync: () => { throw new Error('source hook is not represented by the internal resolver') },
    }
