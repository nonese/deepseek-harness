...
export class AppWebEntry {
  ...
  private shellDispose: (() => void) | undefined
  ...
  async run(): Promise<void> {
    try {
      ...
      const multiUser = (globalThis as { __HARNESS_MULTI_USER__?: unknown }).__HARNESS_MULTI_USER__ === true
      let auth: AuthPreflight | undefined
      if (multiUser) {
        auth = await authPreflight()
        if (!auth.authenticated) {
          this.page.dispose()
          this.shellDispose = mountLogin(this.container, auth.login)
          return
        }
      }
      ...
      if (auth?.authenticated === true) {
        this.page.dispose()
        this.shellDispose = mountAuthenticatedShell(this.container, ctx, auth.session)
      } else {
