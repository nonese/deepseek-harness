...
已认证的服务端部署会在插件运行时之外套用 Harness portal。个人“设置”页面读取并修改当前用户是否启用统一 DeepSeek 凭据；仅管理员可见的“系统设置”页面可以替换或移除服务端凭据并配置或测试 OIDC，但永远不会接收已存储的密钥值。只有当 `/auth/session` 报告完整且启用的 OIDC 配置时，匿名外壳才会启用企业 SSO；回调错误码会转换为受限的用户提示，并且本地登录继续可用。普通“设置”导航不会进入插件运行时。

匿名登录 shell 把半透明表单面板叠放在全画幅的海滨 Harness 插画上。宽视口在右侧保留鲸鱼娘场景，并通过插画中的浅色雾区把它自然过渡到表单区域；窄视口裁切到这片安静的雾区，使表单无需独立图片栏即可保留完整内容宽度。
