...
describe('official browser-brand plugin', () => {
  ...
  it('renders the Harness name independently from both requested monogram sizes', () => {
    const name = render(<OfficialBrandName />)
    expect(name.getByText('Harness').tagName).toBe('SPAN')
    name.unmount()

    const mark = render(<OfficialBrandMark size={34} className="hero-mark" />)
    expect(mark.getByText('H').style.width).toBe('34px')
    expect(mark.getByText('H').className).toBe('hero-mark')
    mark.rerender(<OfficialBrandMark size={24} />)
    expect(mark.getByText('H').style.width).toBe('24px')
  })
})
