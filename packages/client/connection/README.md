...
The browser uses HTTP POST for Remote unary calls. API Gateway owns the `/api/remote.mux` WebSocket and its logical streams; in-process compositions provide equivalent Remote streams through `connection.rpc.open` without opening a WebSocket. The Host half owns the sole `/api` route, Fetch bridge, pluggable deployment authentication, Host/Origin checks, and exact `GET`/`HEAD` route registry. Typert Gateway claims generated Remote endpoints, feature packages register non-JSON responses such as Session-log downloads, and unclaimed requests return 404. Loopback hostname classification remains package-internal to the browser-facing Client state.
...
Every Host RPC method and WebSocket stream requires one browser session; there is no method-specific loopback tier. Connection supplies a single-user process-token authenticator by default. Each process mints a random launch token; `dsh-web-app` opens `?token=...`, and `frontend-static` delegates root and index requests to `ctx.connection.authorizeIndex`, which exchanges it for an authority-bound signed cookie and redirects to clean `/`. A deployment may register one `ConnectionRequestAuthenticator` to replace that default for request authentication, index admission, and the initial browser URL. The multi-user Web bundle uses this seam for its durable Harness session cookie and runs each accepted HTTP or WebSocket dispatch inside the matching request-principal context.
...
Before authentication, every request still passes `src/api-request-trust.ts`. Its `Host` must be loopback or match a `trustedHosts` entry: exact on `host:port`, any port on port-less entries, both sides WHATWG-normalized. An attached `Origin` must equal that Host and `sec-fetch-site: cross-site` is refused. Malformed configured authorities fail plugin load. These checks defend DNS rebinding and cross-site browser requests; they never establish identity. A failed Host/Origin check returns 403, while a trusted but unauthenticated request returns 401. `dsh web --host 0.0.0.0` derives the machine's LAN IPv4 literals into this list; DNS aliases still require `--trusted-host`. Decision records: [browser request trust](../../../.agents/notes/implemented/architecture/2026-07-28-api-browser-trust-boundary.md) and [browser token authentication](../../../.agents/notes/implemented/architecture/2026-08-24-browser-token-authentication.md).
...
- **The default process-token authenticator has no logout route** — a deployment authenticator owns logout and revocation semantics; the multi-user Web bundle provides both.
