...
/** Request authorization that preserves an authenticated execution context. */
export interface ConnectionRequestAuthorization {
  /**
   * Run request dispatch while the authenticator's principal is current.
   * @param operation - transport or business dispatch for this request.
   * @returns the operation result.
   */
  run<T>(operation: () => T): T
}

/** Deployment-owned browser authenticator registered over the process-token default. */
export interface ConnectionRequestAuthenticator {
  /**
   * Resolve one API or WebSocket request.
   * @param request - request headers containing the deployment session cookie.
   * @returns an authenticated execution context, or undefined when login is required.
   */
  authenticate(request: ConnectionTrustRequest): Promise<ConnectionRequestAuthorization | undefined>

  /**
   * Decide whether the public application index may be served.
   * @param request - root or index request.
   * @param response - response owned when the authenticator rejects or redirects.
   * @returns true when index.html may be served.
   */
  authorizeIndex(request: ConnectionIndexRequest, response: ConnectionIndexResponse): Promise<boolean>

  /**
   * Return the URL a launcher should open for this authentication mode.
   * @param baseUrl - clean canonical application URL.
   * @returns a URL accepted by {@link authorizeIndex}.
   */
...
export interface HostConnectionHandle {
  ...
  /**
   * Register one deployment authenticator over the single-user process-token default.
   * @param authenticator - request and index authorization implementation.
   * @returns asynchronous disposer restoring the default authenticator.
   */
  registerAuthenticator(authenticator: ConnectionRequestAuthenticator): () => Promise<void>

  /**
   * Apply the trust fence and resolve the authenticated request context.
   * @param request - request headers from an HTTP or upgrade carrier.
   * @returns an authorization context, or a rejection status.
   */
  authorizeRequest(request: ConnectionTrustRequest): Promise<ConnectionRequestAuthorization | 401 | 403>
