...
import type {
  ...
  ConnectionRequestAuthenticator,
...
export class HostConnectionService extends Service implements HostConnectionHandle {
  ...
  private authenticator: ConnectionRequestAuthenticator | undefined
  ...
  /** Register the sole deployment authenticator over the process-token default. */
  registerAuthenticator(authenticator: ConnectionRequestAuthenticator): () => Promise<void> {
    const owner = this.ctx
    return owner.effect(() => {
      if (this.authenticator !== undefined) {
        throw new Error('connection: deployment authenticator is already registered')
      }
      this.authenticator = authenticator
      return () => {
        if (this.authenticator === authenticator) this.authenticator = undefined
      }
    }, 'client-connection: deployment authenticator')
  }

  /** Apply the trust fence and resolve one request-scoped authorization. */
  async authorizeRequest(
    request: ConnectionTrustRequest,
  ): Promise<ConnectionRequestAuthorization | 401 | 403> {
    ...
    return await authenticator.authenticate(request) ?? 401
  }
  ...
  async authorizeIndex(request: ConnectionIndexRequest, response: ConnectionIndexResponse): Promise<boolean> {
    return this.authenticator === undefined
      ...
      : this.authenticator.authorizeIndex(request, response)
  }

  /** Add this process's launch token to the clean application URL. */
  authenticatedUrl(baseUrl: string): string {
    return this.authenticator === undefined
      ? this.browserAuth.authenticatedUrl(baseUrl)
      : this.authenticator.authenticatedUrl(baseUrl)
  }
    ...
    const route: WebRoute = {
      ...
      handler: async (req, res) => {
        ...
        if (typeof authorization === 'number') {
          ...
          res.end(authorization === 401 ? 'unauthorized' : 'forbidden')
          return
        }
        await authorization.run(() => bridge(req, res, fetchHandler))
      },
    }
    return owner.effect(
