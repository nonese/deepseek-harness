...
describe('connection node half', () => {
  ...
  it('lets one deployment authenticator carry its principal through asynchronous RPC dispatch', async () => {
    const { routes, connection, dispose } = await mounted()
    const principals = new AsyncLocalStorage<string>()
    let authenticateCalls = 0
    const removeAuthenticator = connection.registerAuthenticator({
      authenticate: async (request) => {
        authenticateCalls += 1
        const cookie = request.headers instanceof Headers
          ? request.headers.get('cookie')
          : request.headers.cookie
        return cookie === 'tenant=alice'
          ? { run: operation => principals.run('alice', operation) }
          : undefined
      },
      authorizeIndex: () => Promise.resolve(true),
      authenticatedUrl: baseUrl => baseUrl,
    })
    const removeHandler = connection.rpc.intercept(
      '/api',
      endpoint => endpoint === 'identity/current',
      async () => {
        await Promise.resolve()
        return { ok: true, value: principals.getStore() }
      },
    )
    const request: ClientRequest = {
      type: 'client-request',
      rpcId: RpcId('principal-context'),
      method: 'identity/current',
      payload: { args: {} },
    }
    const allowed = fakeResponse()
    await routes[0]!.handler(fakePost({
      host: '127.0.0.1:3080', cookie: 'tenant=alice',
    }, '/api/identity/current', request), allowed.response)
    expect(JSON.parse(String(allowed.state.body))).toMatchObject({
      result: { ok: true, value: 'alice' },
    })

    const anonymous = fakeResponse()
    await routes[0]!.handler(fakePost({
      host: '127.0.0.1:3080',
    }, '/api/identity/current', request), anonymous.response)
    expect(anonymous.state).toMatchObject({ status: 401, body: 'unauthorized' })
    const beforeUntrusted = authenticateCalls
    await expect(connection.authorizeRequest(fakeRequest({ host: 'untrusted.example' }))).resolves.toBe(403)
    expect(authenticateCalls).toBe(beforeUntrusted)

    await removeHandler()
    await removeAuthenticator()
    await expect(connection.authorizeRequest(fakeRequest({ host: '127.0.0.1:3080' }))).resolves.toBe(401)
    await dispose()
  })
