...
Host composition may register ordered `TypertGatewayMiddleware` around both unary and stream dispatch. Each middleware can reject or project a request and result, must delegate explicitly to continue, and is removed with its owning Cordis effect. Stream middleware wraps the iterable itself, so authorization and redaction apply to every emitted item, including the reserved `$events/follow` stream. The multi-user Web deployment uses this seam after Connection authentication to enforce principal-scoped session, workspace, settings, credentials, plugin-management, and Host-fact access.
