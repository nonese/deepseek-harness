...
Host 组合可以为一元与 stream 分发注册有序 `TypertGatewayMiddleware`。每个中间件都可以拒绝或投影请求与结果，继续处理时必须显式委托，并随所属 Cordis effect 一起移除。Stream 中间件包裹 iterable 本身，因此授权与脱敏会应用到每个产出项，包括保留的 `$events/follow` stream。多用户 Web 部署会在 Connection 认证后使用该 seam，按主体限制会话、workspace、设置、凭据、插件管理与 Host 信息访问。
