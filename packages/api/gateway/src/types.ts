...
/** Effect-scoped authorization or projection around every Remote dispatch. */
export interface TypertGatewayMiddleware {
  /**
   * Authorize or project one unary Remote call.
   * @param request - decoded Remote request.
   * @param next - next middleware or the business invocation.
   * @returns the projected business result.
   */
  invoke(request: InvokeRemoteRequest, next: () => Promise<unknown>): Promise<unknown>

  /**
   * Authorize or project one streaming Remote call.
   * @param request - decoded Remote request.
   * @param next - next middleware or the business stream.
   * @returns the projected stream.
   */
  stream(
    request: InvokeRemoteRequest,
    next: () => Promise<AsyncIterable<unknown>>,
...
export interface TypertGateway {
  ...
  /**
   * Register one ordered authorization or result-projection layer.
   * @param middleware - unary and stream handlers.
   * @returns disposer removing this exact middleware.
   */
