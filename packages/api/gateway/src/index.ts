...
export class TypertGatewayService extends Service implements TypertGateway {
  ...
  private readonly middleware: TypertGatewayMiddleware[] = []

  /**
   * Register one effect-scoped Remote middleware in activation order.
   * @param middleware - unary and stream dispatch wrapper to append.
   * @returns disposer that removes this exact middleware registration.
   */
  registerMiddleware(middleware: TypertGatewayMiddleware): () => Promise<void> {
    const owner = this.ctx
    return owner.effect(() => {
      this.middleware.push(middleware)
      return () => {
        const at = this.middleware.indexOf(middleware)
        if (at !== -1) this.middleware.splice(at, 1)
      }
    }, 'api-gateway: Remote middleware')
  }
  ...
  async invoke(request: InvokeRemoteRequest): Promise<unknown> {
    const dispatch = (index: number): Promise<unknown> => {
      ...
      return middleware === undefined
        ? this.invokeBusiness(request)
        : middleware.invoke(request, () => dispatch(index + 1))
    }
    return dispatch(0)
  }

  private async invokeBusiness(request: InvokeRemoteRequest): Promise<unknown> {
  ...
  async stream(request: InvokeRemoteRequest): Promise<AsyncIterable<unknown>> {
    return this.runStreamMiddleware(request, () => this.streamBusiness(request))
  }

  private runStreamMiddleware(
    request: InvokeRemoteRequest,
    business: () => Promise<AsyncIterable<unknown>>,
  ): Promise<AsyncIterable<unknown>> {
    const dispatch = (index: number): Promise<AsyncIterable<unknown>> => {
      const middleware = this.middleware[index]
      return middleware === undefined
        ? business()
        : middleware.stream(request, () => dispatch(index + 1))
    }
    return dispatch(0)
  }

  private async streamBusiness(request: InvokeRemoteRequest): Promise<AsyncIterable<unknown>> {
    const prepared = await this.prepareInvocation(request)
    if (prepared.descriptor.mode !== 'stream') {
      throw new TypertGatewayError(
        'signature-invalid',
  ...
  private async openWireStream(
    ...
    if (endpoint === REMOTE_EVENT_STREAM_ENDPOINT) {
      return this.runStreamMiddleware(
        { namespace: '$events', method: 'follow', args: remoteEventArgs(payload), signal },
        () => Promise.resolve(this.openRemoteEvents(payload, signal)),
      )
    }
...
function remoteEventArgs(payload: unknown): Readonly<Record<string, unknown>> {
  if (!isObject(payload)
    || !isPlainObject(payload)
    || Reflect.ownKeys(payload).length !== 1
    || !Object.hasOwn(payload, 'args')
