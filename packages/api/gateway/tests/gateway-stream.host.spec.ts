...
describe('Typert Remote streams', () => {
  ...
  it('lets ordered stream middleware project business items', async () => {
    const { ctx } = await setup(false)
    const calls: string[] = []
    const remove = ctx.typertGateway.registerMiddleware({
      invoke: (_request, next) => next(),
      stream: async (_request, next) => {
        calls.push('before')
        const source = await next()
        return (async function *() {
          for await (const item of source) yield `projected:${String(item)}`
        })()
      },
    })
    const stream = await ctx.typertGateway.stream({
      namespace: 'feed', method: 'sync', args: { label: 'one' },
    })
    const items: unknown[] = []
    for await (const item of stream) items.push(item)
    expect(calls).toEqual(['before'])
    expect(items).toEqual(['projected:one:one', 'projected:one:two'])
    await remove()
  })
