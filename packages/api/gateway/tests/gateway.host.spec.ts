...
describe('TypertGatewayService', () => {
  ...
  it('runs ordered unary middleware around business dispatch and removes it by effect', async () => {
    const { ctx } = await setup()
    registerAgentLookup(ctx, { id: 'agent-1' })
    const calls: string[] = []
    const removeOuter = ctx.typertGateway.registerMiddleware({
      invoke: async (_request, next) => {
        calls.push('outer:before')
        const value = await next()
        calls.push('outer:after')
        return { outer: value }
      },
      stream: (_request, next) => next(),
    })
    const removeInner = ctx.typertGateway.registerMiddleware({
      invoke: async (_request, next) => {
    ...
    await expect(ctx.typertGateway.invoke(request)).resolves.toEqual({
      outer: { inner: { agentId: 'agent-1', title: 'ship', scope: 'root' } },
    })
    expect(calls).toEqual(['outer:before', 'inner:before', 'inner:after', 'outer:after'])

    await removeInner()
    await removeOuter()
    await expect(ctx.typertGateway.invoke(request)).resolves.toMatchObject({ title: 'ship' })
  })
