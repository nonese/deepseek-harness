...
The Web administrator API reserves `managed-<site id>` profiles and `HARNESS_SHARED_MODEL_<SITE ID>_API_KEY` references for custom shared sites. A real model request using one of these references resolves the session's project owner and requires that owner's managed-model preference to be enabled before reading the credential. Discovery and administration can describe the route without a session, but cannot use its key for generation. In the authenticated multi-user carrier, every other `apiKeyEnv` resolves from the session owner's personal credential scope; compositions without that carrier retain deployment-wide resolution.
