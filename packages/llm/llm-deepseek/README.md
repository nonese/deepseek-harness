...
In an authenticated server composition, a `deepseek-official` request for `deepseek-v4-flash` may use the dedicated `HARNESS_SHARED_DEEPSEEK_API_KEY` credential when its live session path belongs to an active user who opted in. Every other provider, model, user, sessionless call, and opted-out request follows the ordinary `apiKeyEnv` resolution above. An opted-in request fails with `MISSING_CREDENTIAL` if an administrator removes the managed key; the key never enters the session log or model-visible content.
