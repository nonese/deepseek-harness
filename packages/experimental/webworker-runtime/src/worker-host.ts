 ...
 * @returns Boot patches (preset root overlay, server authentication off,
 * frontend serving off) and whether the preset overlay was applied.
 */
function bootPatches(
  ...
  // The preview is a browser-local, in-memory deployment. Keep the Auth service
  // for the Web startup dependency, but skip Node-only password hashing and HTTP
  // cookie routes; the production Web profile retains both behaviors.
  const authFile = find(rows, 'auth-file')
  if (authFile !== undefined) {
    patches.push({
      id: 'auth-file',
      config: { ...configOf(authFile), bootstrapAdministrator: false },
    })
  }
  if (find(rows, 'auth-web') !== undefined) patches.push({ id: 'auth-web', disabled: true })
