...
运行 `dsh --profile web`，界面会在你的默认浏览器中打开，即可与 agent（智能体）交互式聊天。你会获得会话视图、模型与设置管理以及会话历史，背后与其他表层相同的模型访问、工具与安全默认值。匿名浏览器会看到 Harness 登录页；本地登录或已配置的 OIDC 会建立仅限 HTTP 的浏览器会话。你可以从命令行更改端口、关闭浏览器交接、允许额外主机，或绑定所有网络接口以提供经过认证的局域网访问。需要浏览器中的交互式工作时选择它；`dsh-headless` 是一次性的命令行兄弟表层。
...
默认情况下 GUI 只接受本机的连接。绑定所有网络接口的部署也会允许 LAN 内的浏览器访问，此时打印的 URL 会附带一个 LAN 地址；`--trusted-host` 在两种情况下都能添加额外主机。Host 与 Origin 检查控制可达性，经过认证的本地或 OIDC 会话则保护每个 Host API 方法与 WebSocket stream。LAN 地址只在启动时采样一次，因此之后的网络变化不会被感知——重启 GUI 以重新公告。
