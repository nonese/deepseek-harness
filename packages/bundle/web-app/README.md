...
Run `dsh --profile web` and the interface opens in your default browser, ready for interactive chat with the agent. You get the conversation view, model and settings management, and session history, backed by the same model access, tools, and safety defaults as every other surface. Anonymous browsers see the Harness login page; local login or configured OIDC establishes an HTTP-only browser session. You can change the port, suppress the browser handoff, allow extra hosts, or bind all network interfaces for authenticated LAN access. Choose it for interactive work in the browser; `dsh-headless` is the one-shot command-line sibling.
...
By default the GUI accepts connections from this machine only. A deployment that binds all network interfaces also allows browsers from the LAN, and the printed URL then includes a LAN address; `--trusted-host` adds extra hosts in either case. Host and Origin checks control reachability, while the authenticated local or OIDC session protects every Host API method and WebSocket stream. The LAN addresses are sampled once at startup, so a network change later is not picked up — restart the GUI to re-advertise.
