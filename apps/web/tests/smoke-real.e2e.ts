...
import { REPO_ROOT, connectFreshWorkspace, newChinesePage, newEnglishPage, probeFreePort, requireDist, saveFailureShot } from './support.ts'
...
const SERVER_LOGIN_EXPECTED = fileURLToPath(new URL('./expected/server-auth/login.expected.md', import.meta.url))
const SERVER_ADMIN_EXPECTED = fileURLToPath(new URL('./expected/server-auth/admin.expected.md', import.meta.url))
const SMOKE_ADMIN_PASSWORD = 'harness-smoke-admin-password'
const authenticatedCookies = new Map<string, Promise<{ origin: string; cookie: string }>>()
const managedProjects = new Map<string, Promise<string>>()

/** Log into the real multi-user server once for Node-side HTTP/WebSocket probes. */
function authenticatedWeb(launchUrl: string): Promise<{ origin: string; cookie: string }> {
  ...
  const exchange = (async () => {
    ...
    const response = await fetch(`${origin}/auth/login/local`, {
      ...
      body: JSON.stringify({ username: 'admin', password: SMOKE_ADMIN_PASSWORD }),
    })
    const setCookie = response.headers.get('set-cookie')
    if (response.status !== 200 || setCookie === null) {
...
/** Create one managed project for a real-server test process and return its workspace id. */
function managedWorkspace(baseUrl: string): Promise<string> {
  const existing = managedProjects.get(baseUrl)
  if (existing !== undefined) return existing
  const created = (async () => {
    const authenticated = await authenticatedWeb(baseUrl)
    const response = await fetch(`${authenticated.origin}/auth/projects`, {
      method: 'POST',
      headers: { 'content-type': 'application/json', cookie: authenticated.cookie, origin: authenticated.origin },
      body: JSON.stringify({ name: 'smoke-real-project' }),
    })
    if (response.status !== 201) throw new Error(`project creation returned HTTP ${String(response.status)}`)
    const body = await response.json() as { project?: { id?: string } }
    if (typeof body.project?.id !== 'string') throw new Error('project creation omitted its workspace id')
    return body.project.id
  })()
  managedProjects.set(baseUrl, created)
  return created
}

/** Log the browser into the real server and wait for the authenticated portal. */
async function loginBrowser(page: Page): Promise<void> {
  await page.getByLabel('用户名', { exact: true }).fill('admin')
  await page.getByLabel('密码', { exact: true }).fill(SMOKE_ADMIN_PASSWORD)
  await page.getByRole('button', { name: '登录', exact: true }).click()
  await page.getByRole('button', { name: '新建会话', exact: true }).waitFor({ timeout: 30_000 })
}

/** Compare one real-browser accessibility snapshot or refresh it explicitly. */
function compareServerSnapshot(path: string, value: string): void {
  const payload = `${value}\n`
...
async function remoteRpc<T>(baseUrl: string, endpoint: string, args: object): Promise<T> {
  ...
  if (endpoint === 'session/create') {
    const request = (args as Readonly<Record<string, unknown>>).request
    if (typeof request === 'object' && request !== null
      && (request as Readonly<Record<string, unknown>>).cwd === undefined
      && (request as Readonly<Record<string, unknown>>).workspaceId === undefined) {
      effectiveArgs = {
        ...args,
        request: { ...request as Readonly<Record<string, unknown>>, workspaceId: await managedWorkspace(baseUrl) },
      }
    }
  }
...
describe('dsh web keyless CLI smoke', () => {
  it('serves a usable app from two immutable plugin batches', async () => {
    ...
    const child = spawn(
      ...
      ['--import', tsxLoader, join(REPO_ROOT, 'apps/cli/src/bin.ts'), 'web', '--no-open', '--port', '0'],
      {
        ...
        env: {
          ...
          HARNESS_BOOTSTRAP_PASSWORD: SMOKE_ADMIN_PASSWORD,
    ...
    try {
      ...
      const page = await newChinesePage(browser)
      const pluginScripts: string[] = []
      const remoteRequests: string[] = []
      const remoteSockets: string[] = []
      ...
      page.on('request', (request) => {
        ...
        if (url.pathname.startsWith('/api/')) remoteRequests.push(resource)
      })
      page.on('websocket', (socket) => { remoteSockets.push(socket.url()) })
      ...
      compareServerSnapshot(SERVER_ADMIN_EXPECTED, await page.locator('body').ariaSnapshot())
...
describe.skipIf(!process.env.DEEPSEEK_API_KEY || notReady.length > 0)('web smoke (real host, real key)', () => {
  ...
  beforeAll(async () => {
    ...
    await loginBrowser(page)
    await managedWorkspace(baseUrl)
    await page.reload({ waitUntil: 'load' })
    await page.getByRole('button', { name: /smoke-real-project/ }).click()
