...
export const WELCOME_NOTICE_COPY = {
  zh: {
    title: '内测声明',
    body: 'Harness 目前的 0.1 版本仍处在面向 Harness 开发者进行测试的阶段，还有许多地方需要持续改进和打磨，希望听取广大开发者的反馈建议。预计 Harness 的核心插件以及基础 API 都会在接下来的一段时间内快速迭代、持续演化。\n\n我们期待与全球开发者一起，在开源、开放、可复用、可组合的基础设施之上，共同探索智能上限。欢迎全球 Harness 开发者加入插件生态。',
    continueLabel: '继续',
  },
} as const

const WEB_E2E_BOOTSTRAP_PASSWORD = 'web e2e bootstrap password'
...
async function cleanupScaffoldWorld(ctx: Context, scaffoldRoot: string, persistenceRoot: string): Promise<unknown[]> {
  const failures: unknown[] = []
  await Promise.resolve(ctx.fiber.dispose()).catch((error: unknown) => failures.push(error))
  await rm(scaffoldRoot, { recursive: true, force: true }).catch((error: unknown) => failures.push(error))
...
export async function launchWebScaffold(options: LaunchOptions = {}): Promise<WebScaffold> {
  ...
  const scaffoldRoot = await realpath(await mkdtemp(join(tmpdir(), 'dsh-web-e2e-ws-')))
  ...
  const harnessHome = options.harnessHome ?? join(scaffoldRoot, '.dsh-home')
  ...
  const skillRootEnvironment = {
    ...
    DSH_AGENTS_HOME: join(scaffoldRoot, '.agents-home'),
    DSH_BUNDLED_SKILL_DIR: join(scaffoldRoot, '.bundled-skills'),
    HARNESS_BOOTSTRAP_PASSWORD: WEB_E2E_BOOTSTRAP_PASSWORD,
  }
  ...
  try {
    await bootstrapContext.plugin(FileAuthService, { root: harnessHome })
    const administrator = bootstrapContext.auth.listUsers().find(user => user.role === 'admin' && user.status === 'active')
    if (administrator === undefined) throw new Error('web e2e scaffold: authentication bootstrap created no administrator')
    workspaceCwd = await realpath(bootstrapContext.auth.userPaths(administrator.id).projects)
  } finally {
    await bootstrapContext.fiber.dispose()
  }
  ...
  try {
    ...
    await rm(scaffoldRoot, { recursive: true, force: true }).catch((cleanupError: unknown) => failures.push(cleanupError))
  ...
  const patches: PatchOptions[] = [
    ...
    { id: 'storage-json', config: { root: join(scaffoldRoot, '.dsh-storages') } },
    ...
    // enabled against the same empty temp workspace, preserving the real seam.
    {
      id: 'skill-filesystem',
      config: {
        dshHome: join(scaffoldRoot, '.dsh-home'),
        agentsHome: join(scaffoldRoot, '.agents-home'),
        bundledSkillDir: join(scaffoldRoot, '.bundled-skills'),
        watch: false,
      },
    },
  ...
  try {
    ...
    const login = await fetch(new URL('/auth/login/local', baseUrl), {
      ...
      body: JSON.stringify({ username: 'admin', password: WEB_E2E_BOOTSTRAP_PASSWORD }),
    })
    const setCookie = login.headers.get('set-cookie')
    if (login.status !== 200 || setCookie === null) {
      throw new Error('web e2e scaffold: local login did not return its session cookie')
    }
    cookieHeader = setCookie.split(';', 1)[0] ?? ''
    if (cookieHeader.length === 0) {
      throw new Error('web e2e scaffold: local login returned an empty session cookie')
    }
    const browserLoginPath = `/__dsh-e2e-login/${randomUUID()}`
    authenticatedUrl = new URL(browserLoginPath, baseUrl).href
    ctx.effect(() => ctx.webServer.register({
      kind: 'exact',
      path: browserLoginPath,
      handler(_request, response) {
        response.writeHead(303, { location: '/?view=runtime', 'set-cookie': setCookie })
        response.end()
      },
    }), 'web e2e scaffold: browser login handoff')
    ctx.effect(() => ctx.webServer.register({
      kind: 'exact',
      path: '/__dsh-e2e-workspace',
      handler(request, response) {
        void (async () => {
          const requestedName = new URL(request.url ?? '/', baseUrl).searchParams.get('name') ?? ''
          if (!/^[a-z0-9._-]{1,80}$/iu.test(requestedName)) {
            response.writeHead(400)
            response.end('invalid test workspace name')
            return
          }
          const path = join(workspaceCwd, requestedName)
          await mkdir(path, { recursive: true })
          const workspace = await ctx.workspaceRegistry.create(path, requestedName)
          response.writeHead(200, { 'content-type': 'application/json' })
          response.end(JSON.stringify({ project: { id: workspace.id, name: requestedName, path } }))
        })().catch((error: unknown) => {
          response.writeHead(500)
          response.end(error instanceof Error ? error.message : String(error))
        })
      },
    }), 'web e2e scaffold: stable managed workspace')
  } catch (error) {
    if (process.cwd() !== originalCwd) process.chdir(originalCwd)
    const cleanupFailures = await cleanupScaffoldWorld(ctx, scaffoldRoot, persistenceRoot)
  ...
  return {
    ...
    async close(): Promise<void> {
      ...
      try {
        ...
        failures.push(...await cleanupScaffoldWorld(ctx, scaffoldRoot, persistenceRoot))
      } finally {
        restoreCredentialEnvironment()
        restoreSkillRootEnvironment()
      }
...
function normalizeAria(snapshot: string, workspaceCwd: string, age: boolean): string {
  ...
  return (age ? snapshot.replace(ARIA_AGE, '{{age}}') : snapshot)
    ...
    .replace(/(?<!\d)\d{1,2}\/\d{1,2}(?:\/\d{2,4})?, \d{1,2}:\d{2}(?::\d{2})?(?:\s*[AP]M)?(?!\w)/gi, '{{clock}}')
