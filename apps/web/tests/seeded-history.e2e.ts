...
describe('web e2e: seeded history renders through cold resume', () => {
  ...
  /** Open the seeded conversation when a focused or preceding test left another view active. */
  const openSeededHistory = async (): Promise<void> => {
    if (await page.getByText('DONE', { exact: true }).count() === 1) return
  ...
  it.skipIf(MODE === 'record')('the server open policy keeps its reason across a retry', async () => {
    ...
    try {
      ...
      await expect.poll(() => dialog.innerText(), { timeout: 5_000 })
        .toContain('path open failed: server deployments do not expose host filesystem pickers or native path opening')
