...
describe('web e2e: startup auto-selection', () => {
  ...
  it('keeps the Hero and composer available when the first managed project opens', async () => {
    ...
    expect(await page.getByText('Into the Unknown', { exact: true }).isVisible()).toBe(true)
    expect(await page.locator('[data-composer-input]').first().getAttribute('aria-disabled')).not.toBe('true')
