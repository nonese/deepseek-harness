...
async function bootWeb(
  ...
  const overrides: PatchOptions[] = [
    ...
    // Authentication owns the deployment's durable user root and its Web
    // adapter waits for both disabled transport rows. Neither contributes to
    // agent preset composition, so keep both outside this Host-plane audit.
    { id: 'auth-file', disabled: true },
    { id: 'auth-web', disabled: true },
...
describe('a composition that configures its own preset roots', () => {
  ...
  it('keeps configured roots alongside the always-prepended shipped root', async () => {
    ...
    expect(listed.map(preset => preset.id).sort()).toEqual(['cordis', 'minimal', 'ptc', 'standard', 'teacher', 'team-spec'])
