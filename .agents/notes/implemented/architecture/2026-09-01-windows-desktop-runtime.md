...
`.github/workflows/windows-desktop.yml` builds on a Windows x64 runner in the user fork. The CLI application manifest declares every runtime peer provider reached through its production package closure; the module-fallback traversal follows each pnpm link to the physical package directory before resolving its isolated dependencies. The workflow injects the built workspace packages into a production runtime, verifies the browser plugin version, starts the staged runtime through its copied `node.exe`, and requests the page through its authenticated loopback URL. A hoisted deploy materializes the desktop package and its development dependencies in a separate directory so Electron Forge traverses concrete dependency directories instead of workspace links. Forge creates an unsigned Squirrel `FZFX-DSH-Setup.exe`, and the workflow publishes it with a checksum and build manifest. Manual dispatch uploads an artifact; a `desktop-v*` tag may also create a GitHub Release. The installer pins a server origin and the server's public signing JWK from repository Actions variables. Neither value is a secret.
