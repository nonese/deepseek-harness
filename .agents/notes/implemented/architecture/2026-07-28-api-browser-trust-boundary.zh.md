...
可达性仍由 webserver 的绑定配置（`host: 127.0.0.1 | 0.0.0.0`）控制，这道栅栏仍是混淆代理人防御而不是认证层。发布的 Web profile 已另行组合浏览器会话认证与主体级授权；只有认证能力就绪后，它的启动提供方才允许发布显式的全接口绑定。自定义组合仍需自行负责可达性与认证策略。旧守卫的回环 socket 检查被放弃而非泛化：绑定表达可达性、`trustedHosts` 点名远程权威之后，socket 地址提供不了头部栅栏覆盖不到的任何东西。
...
- 发布的 Web CLI 必须等认证能力就绪后才能发布显式的 `0.0.0.0` 绑定。自定义的无认证全接口组合仍属于可信网络部署，其所有者必须明确接受相应风险。
