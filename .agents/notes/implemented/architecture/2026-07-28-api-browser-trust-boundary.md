...
Reachability remains the webserver binding's policy (`host: 127.0.0.1 | 0.0.0.0`), and this fence remains a confused-deputy defense rather than an authentication layer. The shipped Web profile now composes browser-session authentication and principal-scoped authorization separately; its startup provider waits for that authentication capability before an explicit all-interfaces bind can be published. Custom compositions still own both their reachability and authentication policy. The old guard's loopback-socket check was dropped rather than generalized: with binding expressing reachability and `trustedHosts` naming remote authorities, the socket address adds nothing a header fence does not already cover.
...
- The shipped Web CLI requires its authentication capability before it can publish an explicit `0.0.0.0` bind. A custom unauthenticated all-interfaces composition remains a trusted-network deployment whose risk its owner must accept explicitly.
