...
| `@deepseek-ai/dsh-tool-image-generation` | `collect_generated_image`、`generate_image` | `ctx.tools`、`ctx.imageGeneration`、`ctx.attachments`、`ctx.llm + an image-capable route at execution time`、`a calling Agent workspace` | `tool/call`、`one workspace-relative .png file`、`durable image attachment`、`tool/result` | - | 随产品发布的 Product Design preset 使用 Dreamina 图片 4.0 2K 为这些工具提供后端。生成结果限定在调用方工作区内；待处理任务通过 collect_generated_image 恢复，而不会重复提交。 |
...
恢复一个待处理的图片生成任务，并将其 PNG 发布到当前项目中。严格复用 generate_image 返回的 task id 和输出路径；不要仅因首个任务仍在处理就再次提交生成。
...
来源：[`packages/image/tool-image-generation/src/index.ts`](../packages/image/tool-image-generation/src/index.ts)

### `generate_image`

通过配置的图片提供方在当前项目中生成一张真实 PNG 图片。随产品发布的 Product Design 模式使用 Dreamina 图片 4.0 2K。请依据实测尺寸选择宽高比和项目相对 `.png` 输出路径；切勿用此工具生成占位图。
...
随产品发布的 Product Design preset 使用 Dreamina 图片 4.0 2K 为这些工具提供后端。生成结果限定在调用方工作区内；待处理任务通过 collect_generated_image 恢复，而不会重复提交。
