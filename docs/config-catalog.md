...
Requires: `auth` · `connection` · `credentials` · `llm` · `settings` · `sessionController` · `sessions` · `typertGateway` · `webServer` · `workspaceRegistry`
...
Source: [`packages/host/auth-web/src/index.ts:66`](../packages/host/auth-web/src/index.ts)
