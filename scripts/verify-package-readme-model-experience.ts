...
const SENTENCE_MODEL_EXPERIENCE: Readonly<Record<string, SentenceContract>> = {
  ...
  'packages/bundle/desktop-app': { kind: 'indirect', reason: 'The desktop patch layer delegates model-facing behavior to the browser and shell consumers it mounts.' },
  ...
  'packages/credentials/credentials-windows': { kind: 'indirect', reason: 'The DPAPI provider stores credential values; consumers of ctx.credentials own any model-facing behavior.' },
  ...
  'packages/identity/auth': { kind: 'none', reason: 'The authentication seam controls user access and directories but contributes no model-visible content.' },
  'packages/identity/auth-file': { kind: 'none', reason: 'The file provider stores authentication records and contributes no model-visible content.' },
  'packages/identity/desktop-auth': { kind: 'none', reason: 'The cryptographic library registers no prompt, tool, or model request input.' },
